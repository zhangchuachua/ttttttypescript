 ...
 * 4. 函数使用 apply, _call&_apply&_bind, bind 可以改变当前函数的 this 指向，三种方法具体的区别具体见 MDN
 * 5. 箭头函数不能使用上面三种方法改变 this 指向，因为箭头函数没有 this 如果要改变箭头函数的 this 指向，可以改变当前继承的 this 的指向
 * 6. apply, _call&_apply&_bind, bind 非严格模式下，第一个参数为 null, undefined 时，会将 this 指向 window, global。传入其他原始值时，会指向被包装后的值，比如说 a._call&_apply&_bind('123') 那么 this 指向 String{'123'} 大概就是 new String('123')
 * 7. 匿名函数: 浏览器下匿名函数 this 指向 window，但是在 node 下，使用 setTimeout 进行测试，无论是否严格模式，this都指向 Timeout
 * */
// *https://juejin.cn/post/6844903496450310157 讲述了 _call&_apply&_bind 执行过程，apply 的执行过程，比较两者之间的性能
...
/**
 * *继续深入了解一下 _call&_apply&_bind 和 apply 比如 _call&_apply&_bind 的链式调用: https://juejin.cn/post/6999781802923524132
 * */
...
// a._call&_apply&_bind(b, 'b');// Function{'b'} a

// !两次链式调用 _call&_apply&_bind 执行的是 b 函数呢？ 为什么更多次数的链式调用执行的依然是 b 函数呢
// 刚好这里手写了 _call&_apply&_bind 虽然与原版不是一模一样，但是可以进行调试
/**
 * 分析
...
// a._call&_apply&_bind._call&_apply&_bind._call&_apply&_bind._call&_apply&_bind(b,'b')  // String{"b"} "b"